...
class IoTClientApp:
    ...
    def setup_ui(self):
        ...
        self.elevator_id_label.grid(row=2, column=0, sticky=tk.W)
        self.elevator_id_entry = ttk.Entry(frame, width=10)
        self.elevator_id_entry.grid(row=2, column=1, sticky=tk.W)

        # Налаштування
        self.settings_label = ttk.Label(frame, text=self.texts['settings'])
        self.settings_label.grid(row=3, column=0, sticky=tk.W)

        # Вибір формату дати
        self.date_format_label = ttk.Label(frame, text=self.texts['date_format'])
        self.date_format_label.grid(row=4, column=0, sticky=tk.W)
        self.date_format_entry = ttk.Entry(frame, width=20)
        self.date_format_entry.insert(0, SETTINGS['date_format'])
        self.date_format_entry.grid(row=4, column=1, sticky=tk.W)

        # Вибір часової зони
        self.time_zone_label = ttk.Label(frame, text=self.texts['time_zone'])
        self.time_zone_label.grid(row=5, column=0, sticky=tk.W)
        self.time_zone_var = tk.StringVar(value=SETTINGS['time_zone'])
        ttk.OptionMenu(frame, self.time_zone_var, SETTINGS['time_zone'], "local", "UTC").grid(row=5, column=1, sticky=tk.W)

        # Вибір системи мір
        self.unit_system_label = ttk.Label(frame, text=self.texts['unit_system'])
        self.unit_system_label.grid(row=6, column=0, sticky=tk.W)
        self.unit_system_var = tk.StringVar(value=SETTINGS['unit_system'])
        ttk.OptionMenu(frame, self.unit_system_var, SETTINGS['unit_system'], "metric", "imperial").grid(row=6, column=1, sticky=tk.W)

        # Кнопка для збереження налаштувань
        self.save_settings_button = ttk.Button(frame, text=self.texts['save_settings'], command=self.save_settings)
        self.save_settings_button.grid(row=7, column=1, sticky=tk.E, pady=5)
        ...
        self.start_button.grid(row=8, column=0, sticky=tk.W)
        ...
        self.stop_button.grid(row=8, column=1, sticky=tk.W)

        # Фрейм для кнопок
        control_buttons_frame = ttk.Frame(frame)
        control_buttons_frame.grid(row=10, column=0, columnspan=3, sticky=(tk.W, tk.E))
        ...
        self.apply_simulation_params_button.grid(row=14, column=2, sticky=tk.W)

        # Поле для виводу відправлених даних
        self.log_text = tk.Text(self.root, height=15, width=60, state=tk.DISABLED)
        self.log_text.grid(row=15, column=0, columnspan=2, padx=10, pady=10)
